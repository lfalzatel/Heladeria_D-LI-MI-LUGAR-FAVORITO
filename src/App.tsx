...
export default function App() {
  ...
  // ── FORCE GRAMS MIGRATION TEMPORARY BUTTON ──
  useEffect(() => {
    const btn = document.createElement('button');
    btn.innerText = "Migrar Helado a Gramos";
    ...
    btn.onclick = async () => {
      ...
      try {
        ...
        for (const sDoc of suppliesSnap.docs) {
          const s = sDoc.data();
          if (s.name.toLowerCase() === 'helado') {
            await updateDoc(doc(db, 'supplies', sDoc.id), {
              unit: 'gr',
              purchaseUnit: 'Caja',
              yieldPerUnit: 5000
            });
        ...
        const updateRecipe = (recipe, baseWeight, scoops) => {
          if (!recipe) return false;
          let changed = false;
          const newRecipe = recipe.map(r => {
            if (r.name.toLowerCase() === 'helado') {
              const qty = (scoops || 1) * baseWeight;
              if (r.quantity !== qty || r.unit !== 'gr') {
                changed = true;
                return { ...r, quantity: qty, unit: 'gr' };
              }
            }
            return r;
          });
          return changed ? newRecipe : false;
        };
        ...
        for (const pDoc of prodsSnap.docs) {
          const p = pDoc.data();
          const baseWeight = getBaseWeight(p.name);
          let needsUpdate = false;
          let newP = { ...p };

          if (p.recipe) {
            const r = updateRecipe(p.recipe, baseWeight, p.scoops || 1);
            if (r) { newP.recipe = r; needsUpdate = true; }
          }
          if (p.variants) {
            newP.variants = p.variants.map(v => {
              const r = updateRecipe(v.recipe, baseWeight, v.scoops || 1);
              if (r) { needsUpdate = true; return { ...v, recipe: r }; }
              return v;
            });
          }

          if (needsUpdate) {
            await updateDoc(doc(db, 'products', pDoc.id), newP);
            prodCount++;
          }
        }
        alert('Migración completada. Insumo actualizado y ' + prodCount + ' productos ajustados.');
      } catch (err) {
