...
const backupPath = './dli_backup_2026-06-12.json';
...
const recipeMap = {
  ...
  "Malteada": "Ingredientes de preparación:\n- Leche: 300 mL (o cantidad estándar usada)\n- Helado: 100 GR (porción de helado)\n- Chantilly: Porción estándar\n- 1 Barquillo\n\nSalsas a elegir:\n- Arequipe, lechera, mora o chocolate."
};

let menuProducts = backupData.products || [];
menuProducts = menuProducts.map(p => {
  if (recipeMap[p.name]) {
...
const newMenuData = {
  icecreamFlavors: backupData.icecreamFlavors || [],
  products: menuProducts,
  supplies: backupData.supplies || []
};

fs.writeFileSync(menuPath, JSON.stringify(newMenuData, null, 2), 'utf8');
